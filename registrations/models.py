...
# Statuses that represent an officially submitted application.
# Draft/Incomplete applications (INCOMPLETE) are excluded so that a draft
# never blocks a new application from using the same mobile/aadhaar/email/etc.
# The current workflow is DRAFT -> SUBMITTED only; the legacy codes below are
# kept so pre-existing records still count as officially submitted.
SUBMITTED_STATUS_CODES = ('SUBMITTED', 'PENDING', 'APPROVED', 'REJECTED', 'CORRECTION')
...
class StudentApplication(models.Model):
    ...
    APPLYING_QUALIFICATION_CHOICES = [
        ('12th or equivalent', '12th or equivalent'),
        ('2yrs of 3 yr dip in ECE/EE/CS/IT and allied branches after 10th', '2yrs of 3 yr dip in ECE/EE/CS/IT and allied branches after 10th'),
        ('Previous relevant Qualification of NSQF Level 3', 'Previous relevant Qualification of NSQF Level 3'),
        ('Previous relevant Qualification of NSQF Level 5', 'Previous relevant Qualification of NSQF Level 5'),
        ('Pursing 3rd yr of UG in CS/IT /ECE/EE/allied branches', 'Pursing 3rd yr of UG in CS/IT /ECE/EE/allied branches'),
        ('Completed 3 year of UG in CS/IT/ECE/EE/allied branches', 'Completed 3 year of UG in CS/IT/ECE/EE/allied branches'),
    ]
    ...
    # Contact
    mobile_number = models.CharField(max_length=10, null=True, blank=True, validators=[validate_indian_mobile])
    ...
    area_village_name = models.CharField(max_length=255, null=True, blank=True)

    # Educational
    applying_qualification = models.CharField(max_length=100, choices=APPLYING_QUALIFICATION_CHOICES, default='12th or equivalent')
    ...
    # Identification
    aadhaar_number = models.CharField(max_length=12, null=True, blank=True, validators=[validate_aadhaar])
    abc_id = models.CharField(max_length=12, null=True, blank=True)
    ...
    is_archived = models.BooleanField(default=False, help_text="Archived applications are kept but hidden from routine workflow.")
    ...
    def is_submitted(self):
        """True once the application has been officially final-submitted."""
        return bool(self.status and self.status.code in SUBMITTED_STATUS_CODES)

    @property
    def is_draft(self):
        """True while the application is only an auto-saved/preview draft."""
