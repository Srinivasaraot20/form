...
def validate_indian_mobile(value):
    if not value or not re.match(r"^[1-9]\d{9}$", value):
        raise ValidationError(_('Number must be 10 digits and cannot start with 0.'))
...
def validate_aadhaar(value):
    if not re.match(r'^[1-9]\d{11}$', str(value)):
        raise ValidationError('Aadhaar number must be 12 digits and cannot start with 0.')
