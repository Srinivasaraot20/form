...
class UploadedDocumentInline(admin.TabularInline):
    ...
    class Media:
        css = {
            'all': ('css/admin_uploaded_documents.css',)
        }
        
    readonly_fields = [
        'doc_type_display', 'original_file_link', 'compressed_file_link', 'uploaded_at', 
    ...
    exclude = [
        'doc_type', 'original_file', 'compressed_file', 'original_file_size', 'optimized_file_size', 
    ...
    def doc_type_display(self, obj):
        name_map = {
            ...
            'signature': 'Signature',
            'thumb_impression': 'Left Thumb',
            'abc_screenshot': 'ABC ID',
            'aadhaar_pdf': 'Aadhaar',
            'community_certificate': 'Community Certificate',
            'registration_screenshot': 'Registration Document',
            'supporting_documents': 'Supporting Documents',
            'ex_service_certificate': 'Ex-Service Certificate'
        }
        return name_map.get(obj.doc_type, obj.doc_type.replace('_', ' ').title())
    doc_type_display.short_description = "Doc Type"
    
    def original_file_link(self, obj):
        if obj.original_file:
            import os
            fname = os.path.basename(obj.original_file.name)
            label = "Original PDF" if obj.mime_type == 'application/pdf' else "Original"
            return format_html('<a href="{}" title="{}" target="_blank" rel="noopener">{}</a>', obj.original_file.url, fname, label)
        return "-"
    ...
    def compressed_file_link(self, obj):
        if obj.compressed_file:
            ...
            label = "Compressed PDF" if obj.mime_type == 'application/pdf' else "Optimized"
            return format_html('<a href="{}" title="{}" target="_blank" rel="noopener">{}</a>', obj.compressed_file.url, fname, label)
        return "-"
    compressed_file_link.short_description = "Compressed File"

    def original_size_kb_display(self, obj):
        if obj.original_size_kb:
            return format_html('<span style="white-space: nowrap;">{:,.2f} KB</span>', obj.original_size_kb)
        return "-"
    original_size_kb_display.short_description = "Original Size"

    def optimized_size_kb_display(self, obj):
        if obj.optimized_size_kb:
            return format_html('<span style="white-space: nowrap;">{:,.2f} KB</span>', obj.optimized_size_kb)
        return "-"
    ...
    def dpi_display(self, obj):
        ...
        if dpi_val:
            try:
                parts = dpi_val.split("x")
                if parts[0].strip() == parts[1].strip():
                    return format_html('<span style="white-space: nowrap;">{} DPI</span>', parts[0].strip())
                return format_html('<span style="white-space: nowrap;">{} &times; {} DPI</span>', parts[0].strip(), parts[1].strip())
            except:
    ...
    def mime_type_display(self, obj):
        if obj.mime_type:
            return format_html('<span style="white-space: nowrap;">{}</span>', obj.mime_type)
        return "-"
    ...
    def validation_status(self, obj):
        ...
        if obj.mime_type == 'application/pdf' or canon_dt not in ['passport_photo', 'signature', 'thumb_impression']:
            return format_html('<span style="color: #6c757d; font-weight: bold;">—</span>')
        ...
        if errors:
            err_str = "&#10;".join(errors)
            return format_html('<span style="color: #dc3545; font-weight: bold; cursor: help;" title="{}">? FAIL<br><span style="font-size:10px; font-weight:normal; color:#666;">Hover for details</span></span>', err_str)
        
        pass_tooltip = f"{w}x{h} px&#10;{kb} KB&#10;{dpi_val} DPI"
        return format_html('<span style="color: #198754; font-weight: bold; cursor: help;" title="{}">? PASS</span>', pass_tooltip)
...
class StudentApplicationAdmin(admin.ModelAdmin):
    ...
    actions = ['approve_applications', 'reject_applications', 'mark_verified', 'need_correction', 'export_excel', 'export_pdf', 'export_zip', 'download_documents', 'regenerate_optimized_images']
    
    def regenerate_optimized_images(self, request, queryset):
        ...
        for app in queryset.prefetch_related('documents'):
            for doc in app.documents.all():
                if doc.doc_type in ['passport_photo', 'signature', 'thumb_impression'] and doc.original_file:
                    try:
                        # Re-run current optimization rules
                        if doc.compressed_file:
                            doc.compressed_file.delete(save=False)
                        
                        optimized_data = process_and_optimize_image(doc.original_file, doc.doc_type)
                        
                        doc.compressed_file.save(optimized_data["optimized_file"].name, optimized_data["optimized_file"], save=False)
                        doc.optimized_file_size = optimized_data["optimized_file_size"]
                        doc.image_width = optimized_data["image_width"]
                        doc.image_height = optimized_data["image_height"]
                        doc.mime_type = optimized_data["mime_type"]
                        doc.compression_percentage = optimized_data["compression_percentage"]
                        
                        doc.save()
                        success_count += 1
                    except Exception as e:
                        error_count += 1
                        self.message_user(request, f"Error regenerating {doc.doc_type} for App #{app.application_number}: {str(e)}", level=messages.ERROR)
        
        if success_count > 0:
            self.message_user(request, f"Successfully regenerated {success_count} optimized images.", level=messages.SUCCESS)
        if error_count == 0 and success_count == 0:
            self.message_user(request, "No processable images found in selected applications.", level=messages.WARNING)
    regenerate_optimized_images.short_description = "Regenerate Optimized Images"
